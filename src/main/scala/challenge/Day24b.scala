...
object Day24b extends Challenge {
  ...
  def aStar(start: Point, goal: Point, points: Array[Array[Point]])(costFn: (Point, Point) => Int): Map[Point, Int] = {
    ...
    while (frontier.nonEmpty) {
      ...
      if (current._1 == goal) frontier.clear
        ...
        val neighbors = List(Delta(0, -1), Delta(0, 1), Delta(1, 0), Delta(-1, 0))
          .map(getNext(current._1, _, points)).filter(p => !p.wall)
        for (next <- neighbors) {
  ...
  override def run(): Any = {
    ...
    val paths2 = paths.map (p => p :+ start)
    paths2.toList.map(pathLength(_, distances)).min
  }
